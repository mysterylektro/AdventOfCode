...
def count_orbits(orbits, key, base):
    if key in orbits.keys():
        count = base
        for val in orbits[key]:
            count += count_orbits(orbits, val, base+1)
        return count
    else:
        # Nothing orbits, we just return the base count
        return base


def get_parent_key(orbits, obj):
    for key, val in orbits.items():
        if obj in val:
            return key


def path_to_val(orbits, obj):
    path = []
    while obj != 'COM':
        path.append(obj)
        obj = get_parent_key(orbits, obj)
    return path
...
if __name__ == '__main__':
    ...
    print("Answer to Part 1: " + str(count_orbits(orbits, 'COM', 0)))

    you_path = path_to_val(orbits, 'YOU')
    san_path = path_to_val(orbits, 'SAN')

    intersections = list(set(you_path).intersection(set(san_path)))
    first_san_intersection = min([san_path.index(intersection) for intersection in intersections])
    first_you_intersection = min([you_path.index(intersection) for intersection in intersections])

    steps = first_you_intersection + first_san_intersection - 2
    print("Answer to Part 2: " + str(steps))
