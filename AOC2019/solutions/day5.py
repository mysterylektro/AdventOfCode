...
parameter. For example, the instruction 4,50 would output the value at address 50. Programs that use these
instructions will come with documentation that explains what should be connected to the input and output. The program
...
3. The result of this operation, 33 * 3 = 99, is written according to the third parameter, 4 in position mode,
...
negative: 1101,100,-1,4,0 is a valid program (find 100 + -1, store the result in position 4). The TEST diagnostic
program will start by requesting from the user the ID of the system to test by running an input instruction - provide
...
class IntComputer:
    ...
    def add_function(self, val1, val2, position):
        if self.verbose:
            print("Adding %s to %s to get %s" % (val1, val2, val1 + val2))
        self.set_val(position, val1 + val2)
        return self.pointer + 4

    def multiply_function(self, val1, val2, position):
        if self.verbose:
            print("Multiplying %s by %s to get %s" % (val1, val2, val1 * val2))
        self.set_val(position, val1 * val2)
        return self.pointer + 4

    def insert_function(self, position):
        if self.input_val is None:
            self.paused = True
            if self.verbose:
                print("No input value! Pausing...")
            return self.pointer
        else:
            self.set_val(position, self.input_val)
            self.input_val = None
            return self.pointer + 2

    def retrieve_function(self, value):
        self.output.append(value)
        if self.verbose:
            print("Retrieved value %s" % value)
        return self.pointer + 2

    def jump_if_nonzero(self, test_val, position):
        if test_val != 0:
            if self.verbose:
                print("Test succeeded! Setting pointer to position: %s" % position)
            return position
        else:
            if self.verbose:
                print("Test failed! Skipping to next instruction at position %s." % (self.pointer + 2))
            return self.pointer + 3

    def jump_if_zero(self, test_val, position):
        if test_val == 0:
            if self.verbose:
                print("Test succeeded! Setting pointer to position: %s" % position)
            return position
    ...
    def less_than(self, test_val1, test_val2, position):
        if test_val1 < test_val2:
            if self.verbose:
                print("Less than test succeeded! Setting index %s to 1." % position)
            self.set_val(position, 1)
        else:
            if self.verbose:
                print("Less than test succeeded! Setting index %s to 0." % position)
            self.set_val(position, 0)
        return self.pointer + 4

    def equal_to(self, test_val1, test_val2, position):
        if test_val1 == test_val2:
            if self.verbose:
                print("Equality test succeeded! Setting index %s to 1." % position)
            self.set_val(position, 1)
        else:
            if self.verbose:
                print("Equality test succeeded! Setting index %s to 0." % position)
            self.set_val(position, 0)
        return self.pointer + 4

    def set_val(self, position, value):
        if self.verbose:
            print("Setting value at address %s to %s" % (position, value))
    ...
    def get_functions(self):
        return {1: (self.add_function, ['r', 'r', 'w']),
                2: (self.multiply_function, ['r', 'r', 'w']),
                3: (self.insert_function, ['w']),
                4: (self.retrieve_function, ['r']),
                5: (self.jump_if_nonzero, ['r', 'r']),
                6: (self.jump_if_zero, ['r', 'r']),
                7: (self.less_than, ['r', 'r', 'w']),
                8: (self.equal_to, ['r', 'r', 'w']),
                99: (self.complete, [])}

    def decode_parameters(self, param_types, modes):
        params = []
        for i, param_type in enumerate(param_types):
            index = (self.pointer + 1) + i
            mode = modes[i]
            if param_type == 'r':
                params.append(self.decode_read_param(index, mode))
            elif param_type == 'w':
                params.append(self.decode_write_param(index, mode))
            else:
    ...
    def decode_write_param(self, index, mode):
        if mode == 0:
            ...
            print("WARNING: Write parameters cannot be used with mode 1, defaulting to mode 0...")
            return self.decode_write_param(index, 0)
        else:
            raise ValueError("Unsupported mode.")

    def run(self, input_val=None, verbose=False):
        self.input_val = input_val
        self.halted = False
        self.paused = False
        self.verbose = verbose

        switch = self.get_functions()
        i = 0
        while not self.halted and not self.paused:
            i += 1
            instruction = format(self.get_instruction(), '05')
            opcode = int(instruction[3:])
            func, param_types = switch.get(opcode)
            modes = [int(i) for i in instruction[:3]][::-1][:len(param_types)]
            params = self.decode_parameters(param_types, modes)
            self.pointer = func(*params)
