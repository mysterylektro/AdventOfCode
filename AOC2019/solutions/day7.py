...
provide it the amplifier's phase setting, 3. At its second input instruction, provide it the input signal,
0. After some calculations, it will use an output instruction to indicate the amplifier's output signal.
...
def run_amplifier(instructions, phase, input_val):
    computer = IntComputer(instructions.copy())
    computer.run(input_val=phase)
    computer.run(input_val=input_val)
...
if __name__ == '__main__':
    ...
    for permutation in permutations:
        computers = [IntComputer(instructions.copy()) for i in permutation]
        input_value = 0
        for index, phase in enumerate(permutation):
            computers[index].run(input_val=phase)
            computers[index].run(input_val=input_value)
            input_value = computers[index].output[-1]
        signals.append(input_value)

    print("Answer to Part 1: " + str(max(signals)))
    ...
    for permutation in permutations:
        ...
        while not halted:
            for index, phase in enumerate(permutation):
                computers[index].run(input_val=input_value)
                input_value = computers[index].output[-1]
                halted = computers[index].halted
        signals.append(input_value)

    print("Answer to Part 2: " + str(max(signals)))
