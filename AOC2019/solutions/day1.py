...
delivering presents to other planets! To accurately calculate his position in space, safely align his warp drive,
...
requires 70 more fuel, which requires 21 fuel, which requires 5 fuel, which requires no further fuel. So,
...
def calc_total_fuel(mass):
    ...
    additional_fuel = calc_fuel(fuel_needed)
...
if __name__ == '__main__':
    ...
    with open(input_file) as f:
        fuel = sum([calc_fuel(int(line)) for line in f])

    with open(input_file) as f:
        total_fuel = sum([calc_total_fuel(int(line)) for line in f])

    print('Answer to Part 1: ' + str(fuel))
    print('Answer to Part 2: ' + str(total_fuel))
