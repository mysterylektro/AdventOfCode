...
they're encoded in a special Space Image Format. None of the Elves seem to remember why this is the case. They send
...
if __name__ == '__main__':
    ...
    with open(input_file) as f:
        image_input = f.readline().strip()

    pointer = 0
    layers = []
    while pointer < len(image_input):
        layer = image_input[pointer:pointer+(25*6)]
        layers.append(layer)
        pointer += 25*6
    ...
    print("Answer to Part 1: " + str(counts[index]['1'] * counts[index]['2']))

    output_image = []
    for i in range(6):
        output_image.append([])
        for j in range(25):
            output_image[i].append(2)

    for layer in layers:
        for row in range(6):
            for idx in range(25):
                if output_image[row][idx] == 2:
                    output_image[row][idx] = int(layer[row*25 + idx])
