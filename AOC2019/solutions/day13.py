...
three output instructions specify the x position (distance from the left), y position (distance from the top),
...
instead specifies the new score to show in the segment display. For example, a sequence of output values like -1,0,
12345 would show 12345 as the player's current score.
...
class ArcadeGame:
    ...
    def run(self):
        ...
        y_vals = []
        tile_ids = []
        for idx, output in enumerate(self.computer.output):
            if idx % 3 == 0:
                x_vals.append(output)
            elif idx % 3 == 1:
                y_vals.append(output)
            else:
                tile_ids.append(output)

        for x, y, tile_id in zip(x_vals, y_vals, tile_ids):
            self.game_tiles[(x, y)] = tile_id
...
if __name__ == '__main__':
    ...
    game = ArcadeGame(instructions.copy())
    game.run()

    block_tiles = [tile for tile, tile_id in game.game_tiles.items() if tile_id == 2]
    print("Answer to Part 1: " + str(len(block_tiles)))
