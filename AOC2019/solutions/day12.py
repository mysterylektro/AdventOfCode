...
because 3 < 5). However, if the positions on a given axis are the same, the velocity on that axis does not change for
that pair of moons.
...
def calculate_cycle(moons, axis):
    init_positions = [moon.position[axis] for moon in moons]
    init_velocities = [moon.velocity[axis] for moon in moons]

    for moon in moons:
        moon.apply_gravity(moons, axis)
    for moon in moons:
        moon.apply_velocity(axis)
    new_positions = [moon.position[axis] for moon in moons]
    new_velocities = [moon.velocity[axis] for moon in moons]

    count = 1
    while not (init_positions == new_positions and init_velocities == new_velocities):
        for moon in moons:
            moon.apply_gravity(moons, axis)
        for moon in moons:
            moon.apply_velocity(axis)
...
def calculate_gravity(val1, val2):
    if val1 > val2:
        return 1
    elif val1 < val2:
        return -1
...
class Moon:
    def __init__(self, x, y, z):
        self.init_position = [x, y, z]
        self.init_velocity = [0, 0, 0]
        self.position = [x, y, z]
        self.velocity = [0, 0, 0]
    ...
    def apply_all_gravity(self, moons):
        for axis in range(3):
            self.apply_gravity(moons, axis)

    def apply_gravity(self, moons, axis):
        vel = 0
        for moon in moons:
            vel += calculate_gravity(moon.position[axis], self.position[axis])
        self.velocity[axis] += vel

    def apply_all_velocity(self):
        for axis in range(3):
...
if __name__ == '__main__':
    ...
    for step in range(n_steps):
        for moon in moons:
            moon.apply_all_gravity(moons)
        for moon in moons:
            moon.apply_all_velocity()

    energy = np.sum([moon.total_energy() for moon in moons])
    print("Answer to Part 1: " + str(energy))

    x_cycle = calculate_cycle(moons, 0)
    y_cycle = calculate_cycle(moons, 1)
    z_cycle = calculate_cycle(moons, 2)

    answer = lcm([x_cycle, y_cycle, z_cycle])
