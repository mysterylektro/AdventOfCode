...
On the way to your gravity assist around the Moon, your ship computer beeps angrily about a "1202 program alarm". On
the radio, an Elf is already explaining how to handle the situation: "Don't worry, that's perfectly norma--" The ship
...
these numbers together to get 70. Then, store this value at the output position; here, the output position (3) is at
...
are called the instruction's parameters. For example, in the instruction 1,2,3,4, 1 is the opcode; 2, 3,
...
def opcode1(opcode_list, current_index):
    position1 = opcode_list[current_index + 1]
    position2 = opcode_list[current_index + 2]
    output_position = opcode_list[current_index + 3]
    opcode_list[output_position] = opcode_list[position1] + opcode_list[position2]


def opcode2(opcode_list, current_index):
    position1 = opcode_list[current_index + 1]
    position2 = opcode_list[current_index + 2]
    output_position = opcode_list[current_index + 3]
    opcode_list[output_position] = opcode_list[position1] * opcode_list[position2]
...
def run_opcodes(input_codes):
    switch = {1: opcode1,
              2: opcode2,
              99: None}
    current_index = 0
    func = switch.get(input_codes[current_index])
    while func is not None:
        func(input_codes, current_index)
        current_index += 4
        func = switch.get(input_codes[current_index])
...
if __name__ == '__main__':
    ...
    input_opcodes = opcodes.copy()

    run_opcodes(input_opcodes)
    print('Answer to Part 1: ' + str(input_opcodes[0]))
    ...
    for noun in range(0, 100):
        for verb in range(0, 100):
            input_opcodes = opcodes.copy()
            input_opcodes[1] = noun
            input_opcodes[2] = verb
            run_opcodes(input_opcodes)
            if input_opcodes[0] == desired_value:
                found = True
                break
        if found:
            break
    ...
    if found:
        ...
        print("Error! Could not find an answer for part 2")
