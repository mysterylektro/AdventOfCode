...
def wire_graph(input_instructions):
    wire_graph = []
    xy = [0, 0]
    steps = [0]
    ...
    for instruction in input_instructions:
        ...
        for step in range(distance):
            ...
            wire_graph.append(tuple(xy))

    return wire_graph, steps
...
if __name__ == '__main__':
    ...
    with open(input_file) as f:
        wire_1, wire_1_steps = wire_graph(f.readline().split(','))
        wire_2, wire_2_steps = wire_graph(f.readline().split(','))

    intersections = list(set(wire_1).intersection(set(wire_2)))
    manhattan_dists = [int(abs(x) + abs(y)) for x, y in intersections if x + y != 0]

    print("Answer to Part 1: " + str(min(manhattan_dists)))

    sum_of_steps = [wire_1_steps[wire_1.index(intersection)] + 1 +
                    wire_2_steps[wire_2.index(intersection)] + 1 for intersection in intersections]

    print("Answer to Part 2: " + str(min(sum_of_steps)))
